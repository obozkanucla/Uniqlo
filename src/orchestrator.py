...
def main():
    ...
    scrape_sku_state(conn, log, max_variants=15)# max_variants=get_max_variants())
    ...
    if events:
        conn.executemany("""
                         INSERT INTO uniqlo_events (event_time,
                                                    catalog,
                                                    event_type,
                                                    product_id,
                                                    sku_path,
        ...
        print(
            "rows_on_disk:",
            conn.execute("SELECT COUNT(*) FROM uniqlo_events").fetchone()
        )
        conn.commit()
